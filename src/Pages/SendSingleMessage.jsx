...
const HandleSendSingleMsg = async () => {
  ...
  const components = tmpl.components.map((comp, compIndex) => {
    ...
    if (compType === "HEADER" && ["IMAGE", "VIDEO", "DOCUMENT"].includes(comp.format)) {
      return obj;
    }

    // Handle TEXT header and BODY parameters
    if ((compType === "HEADER" && comp.format === "TEXT") || compType === "BODY") {
    ...
    if (compType === "BUTTON" && Array.isArray(comp.buttons)) {
      ...
      // ✅ Only allow WhatsApp-supported sub_types
      const allowedSubTypes = [
        "QUICK_REPLY", "URL", "CATALOG", "COPY_CODE", "FLOW",
        "MPM", "ORDER_DETAILS", "REMINDER", "VOICE_CALL"
      ];

      if (!allowedSubTypes.includes(subType)) {
        console.warn(`⚠️ Skipping unsupported button type: ${subType}`);
      ...
      if (subType === "QUICK_REPLY") {
        obj.parameters = [{ type: "payload", payload: button.payload || "YES" }];
      } else if (subType === "URL") {
        obj.parameters = [{ type: "text", text: button.url || "https://example.com" }];
      }
    }
  ...
  const filteredComponents = components.filter((comp) => Object.keys(comp).length > 1);
  ...
  try {
    ...
    console.error("SendSingleMsgs Error:", e.response?.data || e.message);
  }
};
    ...
    <div className="p-4">
      <div className="max-w-3xl mx-auto bg-white shadow-md rounded-xl p-6 sidebar">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          />
          <div className="mt-5">
            <label className="text-sm mb-1  text-gray-700">Select Template</label>
            <select
        ...
        {selectedTemplate && (
          <div className="space-y-4 mt-6">
            ...
            {selectedTemplate.components.map((comp, compIndex) => {
              ...
              return (
                <div key={compIndex} className="p-4 bg-gray-50 border border-gray-100 rounded-2xl">
                  <p className="text-sm mb-2">
                    <strong>Type:</strong> {comp.type} <br />
                    <strong>Format:</strong> {comp.format || "TEXT"}
                  </p>

                  {(comp.type === "BODY" || (comp.type === "HEADER" && comp.format === "TEXT")) &&
