...
import SelectInput from "../utils/SelectInput";
import { conductFile, UserCategories } from "../api/ApiServices";
    ...
    try {
      ...
      if (
        ...
        const formatted = usercategories.categories.map((item) => ({
          label: item.name,
          value: item.name, // 👈 send name instead of id
        }));
        ...
        setSelectCategories(allOptions);
      }
    } catch (error) {
...
//   // Always append category
//   formData.append("category", category);

//   // If new category, also append new_category
//   if (category === "Add New Category") {
//     if (!newCategory.trim()) {
...
//   // ✅ Proper console log so csv_file prints filename
...
//     toast.success("Upload Contact successful!");
...
const handleSubmit = async () => {
  ...
  if (category === "Add New Category") {
    ...
    formData.append("category", "Add New Category");
    formData.append("new_category", newCategory);
  } else {
    formData.append("category", category);  // 👈 now category is NAME
    formData.append("new_category", "");    // always send empty
  }
