...
export default function CreateTemplate() {
  ...
  const handleAddButton = () => {
    setButtons([
      ...buttons,
      { type: "Quick Replay", text: "", phoneNumber: "", link: "" },
    ]);
  };
  ...
  const HandleSubmitTemplate = async () => {
    try {
      ...
      // ✅ Convert button state to API payload
      const buttonsPayload = buttons
        .map((btn) => {
          if (btn.type === "Quick Replay" && btn.text.trim()) {
          ...
          if (btn.type === "Link" && btn.link.trim()) {
            return {
              type: "CALL_TO_ACTION",
              text: "View Now",
              url: btn.link,
    ...
    <div className="border border-gray-400 h-auto w-full p-3 rounded-2xl">
      <div className="flex flex-col lg:flex-row gap-5">
        <div className="flex-1">
          ...
          <div className="w-full xl:w-[700px] lg:w-[500px] bg-gray-100 p-5 rounded-2xl">
            ...
            {header_format === "TEXT" ? (
              ...
              <div className="w-full lg:mt-[60px]">
                <div className="ml-30">
                  <Button
                    text="ChooseFile"
                    onClick={handleUploadClick}
              ...
              <div key={index} className="mt-4 p-3 border rounded-lg">
                ...
                {btn.type === "Quick Replay" && (
                  <input
                    ...
                    className="border border-gray-300 px-4 py-2 rounded-md mt-2 block w-[590px]"
                  />
                )}
        ...
        >
          ...
          {/* Preview */}
          {/* <div className="mt-7 p-3 bg-white overflow-hidden flex-wrap rounded-3xl rounded-bl-none">
            {name || headerText || body_text || footer_text || header_media || buttons.length > 0 ? (
              <>
                {name && (
  <p className=" text-gray-400">
    Template Name: <span className="font-bold text-gray-900">{name}</span>
  </p>
)}
                ...
                {buttons.map((btn, i) => (
                  <div key={i} className="mt-2">
                    {btn.type === "Quick Replay" && (
                      <p className="border px-4 py-1 bg-gray-200 rounded-2xl w-fit">
                        {btn.text || "Quick Reply"}
                      </p>
                    )}
                    {btn.type === "Call" && btn.phoneNumber && (
                      <p className="border px-4 py-1 bg-gray-400 text-white rounded-2xl w-fit">
                        Call Now: {btn.phoneNumber}
                      </p>
                    )}
                    {btn.type === "Link" && btn.link && (
                      <a
                        href={btn.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="border px-4 py-1 bg-blue-500 text-white rounded-2xl w-fit"
                      >
                        View Now
                      </a>
                    )}
  ...
  {name || headerText || body_text || footer_text || header_media || buttons.length > 0 ? (
    <>
      ...
      {headerText && (
        <p className="text-gray-400">
          Header Text: <span className="font-bold text-gray-900">{headerText}</span>
        </p>
      )}

      {body_text && (
        <p className="text-gray-400">
          Body Text: <span className="font-bold text-gray-900">{body_text}</span>
        </p>
      )}

      {header_media && (
        <p className="text-gray-400">
          File: <span className="font-bold text-gray-900">{header_media.name}</span>
        </p>
      )}

      {footer_text && (
        <p className="text-gray-400">
          Footer Text: <span className="font-bold text-gray-900">{footer_text}</span>
        </p>
      )}
